...
class Game {
	...
	private hasPlayerWonMarathonMode(
	...
	public toggleBorder = () => {
		...
		this.canvas.getBorderOn()
			? myCanvas?.classList.add('canvas--showBorder')
			: myCanvas?.classList.remove('canvas--showBorder')
	}

	public showGameEventModal = (title: string, message: string): void => {
		setScore(
			createScore(this.player.getScore(), this.mode.getModeParam().name)
		)
	...
	public addEventListeners = (): void => {
		...
		aboutModalLink?.addEventListener(
			'click',
		...
		ballColorSelect?.addEventListener(
			'change',
			this.selectBallColor.bind(this),
			false
		)
		paddleColorSelect?.addEventListener(
			'change',
			this.selectPaddleColor.bind(this),
			false
		)
		brickColorSelect?.addEventListener(
			'change',
			this.selectBrickColor.bind(this),
			false
		)
		// bootstrap events
		...
		pastelOneRadio?.addEventListener(
			'click',
			this.mouseClickHandler.bind(this),
			false
		)
		pastelTwoRadio?.addEventListener(
			'click',
			this.mouseClickHandler.bind(this),
			false
		)
		...
		scoreBoardResetBtn?.addEventListener(
			'click',
			this.resetScoreBoard.bind(this),
			false
		)
		borderColorCheckBox?.addEventListener(
			'change',
			this.toggleBorder.bind(this),
			false
		)
	}
}
