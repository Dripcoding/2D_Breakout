...
class Canvas implements ICanvas {
	...
	public detectEdgeCollisions(
		...
		if (
			this.isCollidingWithTopEdge(ball) ||
			(this.isApproachingBottomEdge(ball, paddleHeight) &&
				this.isCollidingWithPaddle(ball, paddle))
		) {
	...
	private isApproachingBottomEdge(
		ball: IBall,
		paddleHeight: number
	): boolean {
	...
	private isCollidingWithBrick(
		ball: IBall,
		brick: IBrickObject,
		brickGrid: IBrickGrid
	) {
		return (
			this.isCollidingWithBrickLeftAndRightEdge(
				ball.getBallX(),
				brick.x,
				brickGrid.getBrickWidth()
			) &&
			this.isCollidingWithBrickTopAndBottomEdge(
				ball.getBallY(),
				brick.y,
				brickGrid.getBrickHeight()
			)
		)
	}

	private isCollidingWithBrickLeftAndRightEdge(
		ballX: number,
		brickX: number,
		brickWidth: number
	): boolean {
	...
	private isCollidingWithBrickTopAndBottomEdge(
		ballY: number,
		brickY: number,
		brickHeight: number
	) {
